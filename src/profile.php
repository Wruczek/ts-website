...
try { $clientOnline = CacheManager::i()->getClient($cldbid); } catch (\Throwable $e) { $clientOnline = null; }
try { $serverGroups = CacheManager::i()->getServerGroupList(); } catch (\Throwable $e) { $serverGroups = null; }
...
if ($clientOnline !== null) {
    ...
    try { $online["client_version_short"] = (string) \TeamSpeak3_Helper_Convert::versionShort($online["client_version"] ?? ""); } catch (\Throwable $e) { $online["client_version_short"] = null; }
    ...
    if ($serverGroups) {
        foreach ($serverGroups as $sg) {
            if (in_array($sg["sgid"], $online["client_servergroups_list"], true)) {
                ...
                if (strpos($name, "admin") !== false) { $online["is_admin"] = true; break; }
            }
        }
    }
    ...
    } catch (\Throwable $e) { $dbInfo = null; }
    if ($dbInfo) {
        $online["client_totalconnections"] = (int) $dbInfo["client_totalconnections"];
        $online["client_created"] = (int) $dbInfo["client_created"];
        $online["client_lastconnected"] = (int) $dbInfo["client_lastconnected"];
    }
...
if ($data["client"] === null) {
    ...
    if ($dbInfo) {
        ...
        // Minimal fallback to render template without TS data
        $data["client"] = [
            "client_database_id" => $cldbid,
            "client_nickname" => "User #$cldbid",
            "client_created" => 0,
